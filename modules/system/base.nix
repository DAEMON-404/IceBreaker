...
in
{
  ...
  # ============================================================
  # Auto-mount removable media in Thunar (huge win for forensic
  # triage — plug in a USB and it just appears).
  services.gvfs.enable    = true;   # virtual-FS layer (trash, mtp, smb)
  services.udisks2.enable = true;   # block-device daemon (auto-mount)
  services.tumbler.enable = true;   # thumbnails for Thunar
  programs.xfconf.enable  = true;   # required for xfce4-settings-manager

  # Fast `locate` queries — updated nightly by systemd timer
  services.locate = {
    enable      = true;
  ...
  # ============================================================
  # Bluetooth — opt-in toggle
  # Default OFF so headless / VM installs stay clean. Flip both
  # to true (or set in configuration.nix) on a laptop install.
  # ============================================================
  hardware.bluetooth.enable = lib.mkDefault false;
  services.blueman.enable   = lib.mkDefault false;

  # ============================================================
  # Nix store hygiene — automatic GC and store dedup
  # Without these the /nix/store grows unbounded across rebuilds;
  # this keeps it bounded so the dissertation comparison vs Kali's
  # `apt autoremove` story is fair.
  # nix.gc is configured in nix-helpers.nix
  nix.optimise = {
    automatic = true;
    dates     = [ "weekly" ];
  };
  ...
  environment.systemPackages = with pkgs; [
    ...
    # ── IceBreaker built-in benchmark wrapper ──────────────
